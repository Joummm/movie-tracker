...
export default async function ContentPage({
  ...
  if (params.releaseYear) {
    ...
    podcastQuery = podcastQuery.or(`release_year.eq.${releaseYear},podcasts.release_year.eq.${releaseYear}`)
  } else {
    ...
    if (params.day) {
      ...
      query = query.gte("watched_date", startDateStr).lte("watched_date", endDateStr)
      podcastQuery = podcastQuery.gte("watched_date", startDateStr).lte("watched_date", endDateStr)
      ...
      podcastQuery = podcastQuery.gte("watched_date", `${year}-01-01`).lte("watched_date", `${year}-12-31`)
    }
  }
  ...
  if (params.type && params.type !== "all") {
    if (params.type === "podcast") {
      ...
      query = supabase.from("content").select("*").eq("user_id", user.id).eq("id", "00000000-0000-0000-0000-000000000000") // Empty query
      // Podcast episodes will be handled separately
    } else {
      query = query.eq("type", params.type)
      podcastQuery = supabase.from("podcast_episodes").select("*").eq("user_id", user.id).eq("id", "00000000-0000-0000-0000-000000000000") // Empty query
    }
  }

  const [{ data: content = [] }, { data: podcastEpisodes = [] }] = await Promise.all([
  ...
  const years = Array.from(
    new Set(
      allDates
        .map((c: any) => c.watched_date ? new Date(c.watched_date).getFullYear() : null)
  ...
  const releaseYears = Array.from(
    new Set(
      [
        ...(allContentWithRelease || []).map((c: any) => c.release_year || c.series?.release_year),
  ...
  switch (sortType) {
    ...
    case "oldest":
      ...
      break
    case "highest-rated":
      sortedContent = [...sortedContent].sort((a: any, b: any) => (b.rating || 0) - (a.rating || 0))
      break
    case "lowest-rated":
      sortedContent = [...sortedContent].sort((a: any, b: any) => (a.rating || 0) - (b.rating || 0))
      break
    case "newest":
    default:
      sortedContent = [...sortedContent].sort(
