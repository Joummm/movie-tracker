...
async function fetchShortData(shortId: string, userId: string) {
  ...
  if (!short) {
    console.log("Curta não encontrado ou erro:", shortError);
    return null;
  }
  ...
  // Buscar shorts relacionados (mesmo ano, ou shorts recentes)
  const { data: relatedShorts } = await supabase
    .from("content")
...
export default async function ShortPage({
  ...
  // Fetch short data
  const shortData = await fetchShortData(id, user.id);

  if (!shortData) {
    notFound();
  }

  const { short, details, stats } = shortData;
  ...
  const shortStatus = short.watch_status as WatchStatus;
  const statusColor = statusColors[shortStatus] || "bg-gray-500";
  const statusLabel = statusLabels[shortStatus] || "Desconhecido";
    ...
    <div className="min-h-screen bg-linear-to-b from-background to-background/95">
      ...
      <div className="relative">
        ...
        {short.cover_image && (
          <div className="absolute inset-0 h-125 overflow-hidden">
            <div
              ...
              style={{
                backgroundImage: `url(${short.cover_image})`,
        ...
        <main className="container relative mx-auto px-4 md:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-3">
              <Button
                ...
                size="sm"
                asChild
                className="gap-2 hover:bg-orange-500/10 hover:text-orange-500 transition-all duration-300"
              >
                <Link href="/shorts">
                  <ChevronLeft className="h-4 w-4" />
                  <span className="hidden sm:inline">Voltar para curtas</span>
                  <span className="sm:hidden">Voltar</span>
                </Link>
              </Button>
              <div className="h-4 w-px bg-border/50" />
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <span className="hidden sm:inline">Meus Curtas</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                size="sm"
                className="gap-2 bg-linear-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700"
              >
          ...
          {/* Header Principal do Curta */}
          <div className="flex flex-col lg:flex-row gap-8 mb-12">
            {/* Poster */}
            <div className="lg:w-2/5 xl:w-1/3">
              <div className="relative group">
                {/* Glow effect */}
                <div className="absolute -inset-4 bg-linear-to-r from-orange-500/20 via-blue-500/20 to-purple-500/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            ...
            <div className="lg:w-3/5 xl:w-2/3 space-y-8">
              ...
              <div className="space-y-4">
                <div>
                  <Badge
                    variant="outline"
                    className="mb-3 px-3 py-1 text-sm font-medium border-orange-500/20 text-orange-500"
                  >
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-muted-foreground">
                    {short.release_year && (
                      <span className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-500/5 border border-orange-500/10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <Card className="relative overflow-hidden border-border/50 bg-card">
                  <CardContent className="p-6">
                    ...
                    <Progress
                      value={
                        (stats.total_watch_time / (short.duration || 1 * 10)) *
                        100
                      }
                      className="mt-3"
                    />
            ...
            <TabsContent value="history" className="space-y-8">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {details.viewings.length + (short.watched_date ? 1 : 0)}{" "}
                    registros no total
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    ...
                    {short.watched_date && (
                      <div className="flex items-center justify-between p-6 rounded-xl bg-orange-500/5 border border-orange-500/20">
                        <div className="flex items-center gap-4">
                    ...
                    {details.viewings.length === 0 && !short.watched_date && (
                      <div className="text-center py-12">
            ...
            <TabsContent value="related" className="space-y-8">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Outros curtas da sua coleção
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {details.relatedShorts.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                      {details.relatedShorts.map((relatedShort: any) => (
                        <Link
                          key={relatedShort.id}
                          href={`/shorts/${relatedShort.id}`}
                          className="group block"
                        >
                          <Card className="overflow-hidden border hover:shadow-lg transition-all duration-300">
                            <div className="relative aspect-2/3 overflow-hidden">
                              {relatedShort.cover_image ? (
                                <img
                                  src={relatedShort.cover_image}
                                  alt={relatedShort.name}
                            ...
                            <CardContent className="p-4">
                              ...
                              <div className="flex items-center justify-between mt-2">
                                <div className="text-sm text-muted-foreground">
                                  {relatedShort.release_year}
                                </div>
                                {relatedShort.rating &&
                                  relatedShort.rating > 0 && (
                                    <div className="flex items-center gap-1">
                                      <Star className="h-3 w-3 text-yellow-500 fill-yellow-500" />
                                      <span className="text-sm font-medium">
                                        {relatedShort.rating.toFixed(1)}
                                      </span>
                                    </div>
                                  )}
                              </div>
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-muted-foreground">
                        Nenhum curta relacionado encontrado
                      </p>
                    </div>
                  )}
                </CardContent>
          ...
          <div className="mt-12 pt-8 border-t border-border/50 text-center">
            <p className="text-sm text-muted-foreground">
              Curta adicionado em{" "}
              {new Date(short.created_at).toLocaleDateString("pt-BR")} • Última
              atualização em{" "}
              {new Date(short.updated_at).toLocaleDateString("pt-BR")}
            </p>
          </div>
        </main>
      </div>
