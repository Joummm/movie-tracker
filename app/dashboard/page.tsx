...
export default function DashboardPage() {
  ...
  async function loadDashboardData() {
    ...
    if (!selectedYears.includes("all")) {
      ...
      const { data: allContent } = await query.order("watched_date", { ascending: false })
  ...
  function processStats(allContent: any[], profile: any) {
    ...
    const episodes = allContent?.filter((c) => c.type === "episode").length || 0
    const shorts = allContent?.filter((c) => c.type === "short").length || 0
    const other = allContent?.filter((c) => c.type === "other").length || 0

    // Calculate total watch time
    const totalMinutes = allContent?.reduce((acc, c) => acc + (c.duration || 0), 0) || 0
    const totalHours = Math.floor(totalMinutes / 60)

    // Calculate average rating
    const ratedContent = allContent?.filter((c) => c.rating) || []
    const avgRating =
      ratedContent.length > 0 ? ratedContent.reduce((acc, c) => acc + (c.rating || 0), 0) / ratedContent.length : 0

    // Get unique series count
    const uniqueSeries = new Set(allContent?.filter((c) => c.series_id).map((c) => c.series_id))
    ...
    const monthsData = Array.from({ length: 6 }, (_, i) => {
      ...
      const count =
        allContent?.filter((c) => {
          ...
          return watchedDate.getMonth() + 1 === month && watchedDate.getFullYear() === year
        }).length || 0
