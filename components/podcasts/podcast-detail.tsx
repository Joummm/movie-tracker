        ...
        <div className="lg:col-span-1 space-y-6">
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <div className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>{stats.listenedEpisodes}/{stats.totalEpisodes} episódios ouvidos</span>
                  </div>
                  
                  {stats.totalDuration > 0 && (
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4" />
                      <span>Tempo total: {formatDuration(stats.totalDuration)}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          {podcast.description && (
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground whitespace-pre-line">{podcast.description}</p>
              </CardContent>
            </Card>
          )}
              ...
              {podcast.episodes && podcast.episodes.length > 0 ? (
                <div className="space-y-4">
                  {podcast.episodes
                    .sort((a, b) => {
                      ...
                      if (a.season !== b.season) return (b.season || 1) - (a.season || 1)
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-sm font-medium text-muted-foreground">
                                  S{episode.season || 1}E{episode.episode_number?.toString().padStart(2, '0') || '??'}
                                </span>
                              ...
                              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                                ...
                                {episode.watched_date && (
                                  <span className="flex items-center gap-1">
                                    ...
                                    {new Date(episode.watched_date).toLocaleDateString("pt-PT")}
                                  </span>
                                )}
                              </div>
                              
                              {episode.notes && (
                                <p className="text-sm mt-2 line-clamp-2">{episode.notes}</p>
                              )}
                            </div>
                            ...
                            <div className="flex flex-col gap-2 ml-4">
                              <Button
                                ...
                                asChild
                              >
                                <Link href={`/podcast-episodes/edit/${episode.id}`}>
                              ...
                              {!episode.watched_date && (
                                <Button
                                  ...
                                  asChild
                                >
                                  <Link href={`/podcast-episodes/${episode.id}/listen`}>
                ...
                <Card>
                  <CardContent className="pt-6 text-center">
                    ...
                    <p className="text-lg text-muted-foreground">Nenhum episódio adicionado</p>
                  ...
                  <div className="grid gap-6 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-3xl font-bold">{stats.totalEpisodes}</span>
                        <span className="text-sm text-muted-foreground">total</span>
                      </div>
                      <p className="text-sm text-muted-foreground">
                        {stats.listenedEpisodes} ouvidos • {stats.totalEpisodes - stats.listenedEpisodes} por ouvir
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-3xl font-bold">
                          {stats.totalEpisodes > 0 ? Math.round((stats.listenedEpisodes / stats.totalEpisodes) * 100) : 0}%
                        </span>
                      </div>
            ...
            <TabsContent value="quick-add">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Registe rapidamente um episódio ouvido</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Número do Episódio</label>
                      <input
                        type="number"
                        min="1"
                        className="w-full p-2 border rounded"
                        placeholder="ex: 42"
                        defaultValue={podcast.episodes ? podcast.episodes.length + 1 : 1}
                      />
                    </div>
                    
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Temporada (opcional)</label>
                      <input
                        type="number"
                        min="1"
                        className="w-full p-2 border rounded"
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Nome do Episódio (opcional)</label>
                      <input
                        type="text"
                        className="w-full p-2 border rounded"
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        <label className="text-sm font-medium">Avaliação (0-10)</label>
                        <input
                          type="number"
                          min="0"
                          max="10"
