// components/shorts/ShortForm.tsx
"use client";
...
export function ShortForm({
  ...
  const [imageUrl, setImageUrl] = useState(short?.cover_image || "");
  const [imagePreview, setImagePreview] = useState(short?.cover_image || "");
  const [datePrecision, setDatePrecision] = useState<
    "unknown" | "year" | "month" | "day"
  >(short?.watched_date ? "day" : "unknown");
  const [watchedDate, setWatchedDate] = useState<Date | undefined>(
    short?.watched_date ? new Date(short.watched_date) : undefined,
  );
  ...
  const [formData, setFormData] = useState(() => ({
    name: short?.name || "",
    release_year: short?.release_year || "",
    duration: short?.duration || "",
    rating: short?.rating || "",
    watch_status: short?.watch_status || "planned",
    review: short?.review || "",
    notes: short?.notes || "",
    would_recommend: short?.would_recommend || false,
    would_rewatch: short?.would_rewatch || false,
  }));
  ...
  const roleOptions = useMemo(
    () => [
      ...
      { value: "sound_designer", label: "Designer de Som" },
      { value: "art_director", label: "Diretor de Arte" },
      { value: "other", label: "Outro" },
    ],
    [],
  );

  // Função para processar gêneros
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Adicionar imagem
      if (imageUrl.trim()) {
        shortData.cover_image = imageUrl;
      }

      // Campos numéricos
      if (formData.release_year) {
        const year = parseInt(formData.release_year);
        if (!isNaN(year)) shortData.release_year = year;
      }

      if (formData.duration) {
        const duration = parseInt(formData.duration);
        if (!isNaN(duration)) shortData.duration = duration;
      }

      if (formData.rating) {
        const rating = parseFloat(formData.rating);
        if (!isNaN(rating)) shortData.rating = rating;
      }
      ...
      if (datePrecision !== "unknown" && watchedDate) {
        ...
        shortData.watched_date = dateString;
      }
      ...
      if (isEditing) {
        ...
        await Promise.all([
          supabase.from("content_actors").delete().eq("content_id", shortId),
          supabase.from("content_crew").delete().eq("content_id", shortId),
          supabase.from("content_genres").delete().eq("content_id", shortId),
        ]);
      } else {
        // Create new short
      ...
      toast.success(isEditing ? "Curta atualizado!" : "Curta adicionado!");
      router.push(`/shorts/${shortId}`);
      router.refresh();
    } catch (error: any) {
      console.error("Erro:", error);
      toast.error(error.message || "Erro ao salvar curta");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                {
                  name: "Short Film Central",
                  url: "https://shortfilmcentral.com/",
                  color: "bg-green-500",
                },
                {
                  name: "FilmFreeway",
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Informações essenciais sobre o curta-metragem
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <Label htmlFor="name">Nome do Curta *</Label>
              <Input
                id="name"
                name="name"
                value={formData.name}
                onChange={handleInputChange}
                placeholder="Ex: Piper (Pixar)"
                required
              />
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Curtas-metragens geralmente têm menos de 40 minutos
              </p>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Diretor de Animação"
                  />
                </div>
              </div>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-3">
            ...
            <Textarea
              ...
              onChange={handleInputChange}
              placeholder="Escreva sua resenha sobre o curta-metragem..."
              rows={4}
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            <Textarea
              ...
              onChange={handleInputChange}
              placeholder="Outras anotações sobre o curta-metragem..."
              rows={3}
            />
          </div>
        ...
        <div className="flex items-center gap-4">
          {isEditing && (
            <Button
              ...
              disabled={loading}
              onClick={async () => {
                if (confirm("Tem certeza que deseja excluir este curta?")) {
                  try {
                    setLoading(true);
                    const { error } = await supabase
                      .from("content")
                    ...
                    toast.success("Curta excluído com sucesso!");
                    router.push("/shorts");
                    router.refresh();
                  } catch (error: any) {
                    toast.error(error.message || "Erro ao excluir curta");
                  } finally {
                    setLoading(false);
                  }
                }
          ...
          >
            {loading ? (
              ...
              <>
                ...
                {isEditing ? "Atualizar Curta" : "Adicionar Curta"}
              </>
            )}
          </Button>
        </div>
